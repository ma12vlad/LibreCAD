...
//    snap_menu->addActions(snap_toolbar->actions());
    ...
    QAction* comands = new QAction(QObject::tr("Short commands"), main_window);
    connect(comands, SIGNAL(triggered()), main_window, SLOT(invokeScommandWindow()));
    help_menu->addAction(comands);
    ...
    QMenu* widgets_menu = new QMenu(QC_ApplicationWindow::tr("Widgets"), menu_bar);
    widgets_menu->setTearOffEnabled(true);

    // <[~ Dock Areas Menu ~]>

    QMenu* dockareas_menu = new QMenu(QC_ApplicationWindow::tr("Dock Areas"), widgets_menu);
